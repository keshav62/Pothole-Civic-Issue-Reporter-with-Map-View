  ...
  return (
    <>
      <button
        ...
        className={`fixed inset-0 z-30 bg-slate-950/45 backdrop-blur-[1px] transition-opacity md:hidden ${isOpen ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
      />
      <aside aria-label="Primary navigation" className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 text-slate-300 flex flex-col transition-transform duration-200 ease-in-out border-r border-slate-800 shadow-2xl shadow-slate-950/20 ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                {role === 'CITIZEN' ? 'CITIZEN PORTAL' : 'AUTHORITY PORTAL'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          <div className="px-3 pb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">
            {role === 'SUPER_ADMIN' ? 'Headquarters Admin' : role === 'DEPARTMENT_ADMIN' ? 'Department Control' : role === 'CITIZEN' ? 'Citizen Services' : 'Field Operations'}
          </div>
          {navItems.map((item) => {
        ...
        <div className="p-3 border-t border-slate-800 bg-slate-950/60 space-y-2">
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-800/60 border border-slate-700/50">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400 font-medium truncate">{currentUser?.roleLabel || 'Official'}</p>
            </div>
          </div>
